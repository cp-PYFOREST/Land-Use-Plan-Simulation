...
#saveRDS(results, '~/../../capstone/pyforest/data/results_5608/results5608_5.rds')
...
results5608 <- readRDS("/Users/romero61/../../capstone/pyforest/data/results_5608/results5608_5.rds")
...
rework_over5 <- limit_lu |> 
  filter(cat %in%  optimal_5$cat)

limit_lu <- rework_over5 #run 5572 properties paddocks = 990000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk5.rds')
optimalrwk_result <- readRDS('~/../../capstone/pyforest/data/rework/results_rwk5.rds')
optimal_rwk <- optimalrwk_result %>% 
  group_by(cat) %>%
  arrange(cat, desc(paddocks_area), desc(fr_area)) %>%
  slice(1) %>%
  ungroup()  %>%
  separate(ratio_xy, into = c("width_paddock", "height_paddock"), sep = "/", convert = TRUE, remove = FALSE) # 5572 properties

# Second Subset 
optimal_8_2 <- optimal_rwk |> # I forgot the 3% error on first run
  filter(fr_per > 8)

#record of which properties to be reworked
rework_over8_2 <- limit_lu |> 
  filter(cat %in%  optimal_8_2$cat)

limit_lu <- rework_over8_2 #run 272 properties paddocks = 980000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_2.rds')
optimalrwk_result2 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_2.rds')
optimal_rwk2 <- optimalrwk_result2 %>% 
  group_by(cat) %>%
  arrange(cat, desc(paddocks_area), desc(fr_area)) %>%
  slice(1) %>%
  ungroup() %>%
  separate(ratio_xy, into = c("width_paddock", "height_paddock"), sep = "/", convert = TRUE, remove = FALSE) # 272 properties

# Third Subset 
optimal_8_3 <- optimal_rwk2 |> 
  filter(fr_per > 8) 

#record of which properties to be reworked
rework_over8_3 <- limit_lu |> 
  filter(cat %in%  optimal_8_3$cat)

limit_lu <- rework_over8_3 # run 170 properties Paddocks =950000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_3.rds')
optimalrwk_result3 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_3.rds')
optimal_rwk3 <- optimalrwk_result3 %>% 
  group_by(cat) %>%
  arrange(cat, desc(paddocks_area), desc(fr_area)) %>%
  slice(1) %>%
  ungroup() %>%
  separate(ratio_xy, into = c("width_paddock", "height_paddock"), sep = "/", convert = TRUE, remove = FALSE)# 170 properties


# FOURTH Subset 
optimal_8_4 <- optimal_rwk3 |> 
  filter(fr_per > 8) 

#record of which properties to be reworked
rework_over8_4 <- limit_lu |> 
  filter(cat %in%  optimal_8_4$cat)

limit_lu <- rework_over8_4 # run 57 properties Paddocks = 900000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_4.rds')
optimalrwk_result4 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_4.rds')
optimal_rwk4 <- optimalrwk_result4 %>% 
  group_by(cat) %>%
  arrange(cat, desc(paddocks_area), desc(fr_area)) %>%
  slice(1) %>%
  ungroup() %>%
  separate(ratio_xy, into = c("width_paddock", "height_paddock"), sep = "/", convert = TRUE, remove = FALSE)# 57 properties

# FIFTH Subset 
optimal_8_5 <- optimal_rwk4 |> 
  filter(fr_per > 8)

#record of which properties to be reworked
rework_over8_5 <- limit_lu |> 
  filter(cat %in%  optimal_8_5$cat)

limit_lu <- rework_over8_5 # run 8 properties Paddocks = 800000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_5.rds')
optimalrwk_result5 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_5.rds')
...
# Sixth Subset 
optimal_8_6 <- optimal_rwk5 |> 
  filter(fr_per > 8)

#record of which properties to be reworked
rework_over8_6 <- limit_lu |> 
  filter(cat %in%  optimal_8_6$cat)

limit_lu <- rework_over8_6 # run 2 properties Paddocks = 700000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_6.rds')
optimalrwk_result6 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_6.rds')
...
# Seventh Subset 
optimal_8_7 <- optimal_rwk6 |> 
  filter(fr_per > 8)

#record of which properties to be reworked
rework_over8_7 <- limit_lu |> 
  filter(cat %in%  optimal_8_7$cat)

limit_lu <- rework_over8_7 # run 1 properties Paddocks = 650000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_7.rds')
optimalrwk_result7 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_7.rds')
...
# last Subset 
optimal_8_8 <- optimal_rwk7 |> 
  filter(fr_per > 8)

#record of which properties to be reworked
rework_over8_8 <- limit_lu |> 
  filter(cat %in%  optimal_8_8$cat)

limit_lu <- rework_over8_8 # run 1 properties Paddocks = 600000
```
...
#saveRDS(results, '~/../../capstone/pyforest/data/rework/results_rwk8_8.rds')
optimalrwk_result8 <- readRDS( '~/../../capstone/pyforest/data/rework/results_rwk8_8.rds')
...
optimal_8_2_r <- optimal_rwk |> 
  filter(fr_per <= 8)

optimal_8_3_r <- optimal_rwk2 |> 
  filter(fr_per <= 8) 

optimal_8_4_r <- optimal_rwk3 |> 
  filter(fr_per <= 8) 

optimal_8_5_r <- optimal_rwk4 |> 
  filter(fr_per <=8)

# Sixth Subset 
optimal_8_6_r <- optimal_rwk5 |> 
  filter(fr_per <= 8)

optimal_8_7_r <- optimal_rwk6 |> 
  filter(fr_per <= 8)

optimal_8_8_r <- optimal_rwk7 |> 
  filter(fr_per <= 8)

optimal_rwk8

# add in paddock size for those properties
optimal_8_r <- optimal_8_r %>% mutate(paddock_size = 999999)
optimal_8_2_r <- optimal_8_2_r %>% mutate(paddock_size = 990000)
optimal_8_3_r <- optimal_8_3_r %>% mutate(paddock_size = 980000)
optimal_8_4_r <- optimal_8_4_r %>% mutate(paddock_size = 950000)
optimal_8_5_r <- optimal_8_5_r %>% mutate(paddock_size = 900000)
optimal_8_6_r <- optimal_8_6_r %>% mutate(paddock_size = 800000)
optimal_8_7_r <- optimal_8_7_r %>% mutate(paddock_size = 700000)
optimal_8_8_r <- optimal_8_8_r %>% mutate(paddock_size = 650000)
optimal_rwk8 <- optimal_rwk8 %>% mutate(paddock_size = 600000)

optimal_xy <- bind_rows(optimal_8_r, optimal_8_2_r, optimal_8_3_r, optimal_8_4_r, optimal_8_5_r, optimal_8_6_r, optimal_8_7_r, optimal_8_8_r, optimal_rwk8)
...
optimal_xy_with_duplicates <- bind_rows(optimal_8_r, optimal_8_2_r, optimal_8_3_r, optimal_8_4_r, optimal_8_5_r, optimal_8_6_r, optimal_8_7_r, optimal_8_8_r, optimal_rwk8)
...
optimal_xy
#saveRDS(optimal_xy, '/Users/romero61/../../capstone/pyforest/data/results_5608/optimal_xy_5.rds')
```
...
#save 
#
#st_write(optimal_mock_properties, '/Users/romero61/../../capstone/pyforest/data/optimal_mock_properties/optimal_mock_properties_5.gpkg')
```
